...
La intensidad de las fluctuaciones de velocidad se mide con la velocidad
cuadrática media $u_{\rm rms} = \langle u_1^2\rangle^{1/2}$ (por
componente). La tasa media de disipación de energía cinética por unidad de
masa es
\begin{equation}
  \varepsilon = \nu \left\langle \mathrm{tr}\!\left(\mathbb{A}^{\mathsf{T}}\mathbb{A}\right)\right\rangle,
  \qquad A_{ij} = \frac{\partial u_i}{\partial x_j}.
\end{equation}
La escala integral $L$ es la escala espacial más grande del sistema, a la
cual se inyecta la energía:
\begin{equation}
  L = \frac{u_{\rm rms}^3}{\varepsilon}.
\end{equation}
...
\begin{equation}
  \tau_L = \frac{L}{u_{\rm rms}} = \frac{u_{\rm rms}^2}{\varepsilon}.
\end{equation}
...
domina, $\tau_\eta$ es el tiempo de giro asociado a esa escala, y $u_\eta$
la velocidad característica correspondiente (nótese que $\eta = u_\eta \tau_\eta$
y que $\tau_\eta$ coincide con la extrapolación de $\tau_\ell$ del rango
inercial hasta $\ell=\eta$).
